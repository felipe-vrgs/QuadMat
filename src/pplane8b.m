...
% pplane8b  is a fix in order to run into Matlab 2015.
...
if strcmp(action,'initialize')
   ...
   % First we make sure that there is no other copy of pplane8b
   % running, since this causes problems.	
   pph = findobj('tag','pplane8b');
   if ~isempty(pph);
      qstring = {'There are some pplane8b figures open although they may be invisible.  ';...
            'What do you want to do?'};
      tstring = 'Only one copy of pplane8b can be open at one time.';
      b1str = 'Restart pplane8b.';
   ...
   % Next we look for old files created by pplane8b.
   ...
   for k = 1:length(oldfiles)
      ...
      ll = fgetl(fid);
      fclose(fid);
      if strcmp(ll,'%% Created by pplane8b')
         delete(fn)
      else
	l= length(ofiles);
	ofiles{l+1} = fn;
  ...
  ud.color = color;
  ud.level = ' ';
  ppset = figure('name','pplane8b Setup','NumberTitle','off',...
		 'tag','pplane8b','visible','off',...
		 'UserData',ud);
  
  pplane8b('figdefault',ppset);
  ...
  butt(2) = uicontrol('style','push',...
		      'pos',rwind,...
		      'string','Revert',...
		      'call','pplane8b(''revert'')',...
		      'visible','off');
  
  butt(3) = uicontrol('style','push',...
		      'pos',pwind,...
		      'string','Proceed',...
		      'call','pplane8b(''proceed'')',...
		      'visible','off');
  ...
  callnfpts = [
      'ppset = findobj(''name'',''pplane8b Setup'');',...
...
elseif strcmp(action,'loadsyst')  % This loads either a system or a gallery.
   ...
   for j = 1:nnn
      ...
      newsyst.name = sname;
      ignore = pplane8b('addgall',newsyst);
      if ignore == -1;
...
elseif strcmp(action,'proceed')
   ...
   sud.o = sud.c;
   set(ppset,'UserData',sud);
   he = findobj('name','pplane8b Equilibrium point data');
   hl = findobj('name','pplane8b Linearization');
   close([he;hl]);
   % set([he;hl],'vis','off');
   ...
   if (sud.flag == 1)
     ...
     NumbFPts = sud.c.npts;
     ppdisp = findobj('name','pplane8b Display');
...
%       set([arr.hx;arr.hy;arr.barrows],'vis','on');
	 ...
	 if isempty(pv) 
	   ...
	   if isempty(str2num(pv)) % This is an expression.
	     ...
	     if (err | isempty(pval))
	       ...
	       fprintf('\a')
	       errordlg(errmsg,'pplane8b error','on');
	       warning(SS)
	       return
	     end
	   end
	   xxderivstr = pplane8b('paraeval',pn,pv,xderivstr);
	   yyderivstr = pplane8b('paraeval',pn,pv,yderivstr);
     ...
     % Find pplane8b Display if it exists.	
     % If pplane8b Display exists, update it.  If it does not build it.
	 ...
	 if ~isempty(menull)
       ...
       ppdisp = figure('name','pplane8b Display',...
		       'NumberTitle','off',...
		       'interrupt','on',...
		       'visible','off',...
		       'tag','pplane8b');
       pplane8b('figdefault',ppdisp);
       dud = get(ppdisp,'UserData');
       dud.syst = sud.c;
	...
	otherwise
	 ...
	 set(gcf,'defaultuicontrolfontsize',fs*0.9);
       ...
       merestart = uimenu(fmenu,'label',...
			  ...
			  'call','pplane8b(''restart'')',...
			  'separator','on');
       mequit = uimenu(fmenu,'label','Quit pplane8b',...
		       'call','pplane8b(''quit'')','separator','off');
       ...
       menu(2) = uimenu(emenu,'label','Zoom in.',...
			'call','pplane8b(''zoomin'')',...
			'pos',1);
       
       zsqmenu = uimenu(emenu,'label','Zoom in square.',...
			'call','pplane8b(''zoominsq'')',...
			'pos',2);
	...
	ode113ch = 'off';
	 ...
	 ode23tbch = 'off';
       ...
       menu(6) = uimenu(menu(1),'label','Make the Display Window inactive.',...
			'call','pplane8b(''hotcold'')','separator','on');
	   ...
	   'call','pplane8b(''showbar'')');
       
       set(0,'showhiddenhandles',hhsetup);
       
       set(gcf,'WindowButtonDownFcn','pplane8b(''down'')');
       set(ppdisp,'WindowButtonMotionFcn','pplane8b(''cdisp'')');
	...
	case 6
     ...
     ppkbd = findobj('name','pplane8b Keyboard input','vis','on');
     if ~isempty(ppkbd),pplane8b('kbd'),end
     pplevel = findobj('name','pplane8b Level sets');
...
elseif strcmp(action,'linear')
   ...
   % Get the information from pplane8b Equilibrium ...
   ...
   % Find pplane8b Linearization if it exists.	
   % If pplane8b Linearization exists, update it.  If it does not build it.
   
   pplin= findobj('name','pplane8b Linearization');
   if (~isempty(pplin))
      figure(pplin);
      ...
      pplin = figure('name','pplane8b Linearization',...
         'NumberTitle','off',...
         'interrupt','on',...
         'visible','off',...
         'tag','pplane8b');
      pplane8b('figdefault',pplin);
      dud = get(pplin,'UserData');
      dud.dir = 0;
      dud.syst = system;
      dud.settings = settings;
      ...
      mequit = uimenu(fmenu,'label','Quit pplane8b',...
	  'call','pplane8b(''quit'')','separator','on','pos',9);
	...
	chkd = 'on';
      ...
      set(pplin,'WindowButtonDownFcn',['pplane8b(''down'',',num2str(pplin),')']);
...
elseif strcmp(action,'dirfield')
   ...
   % Find pplane8b Display and get the user data.
   ...
   N = dud.syst.npts;
   if strcmp(get(disph,'name'),'pplane8b Linearization') 
      N = floor(3*N/4);
   end    
...
elseif strcmp(action,'hotcold')
   ...
   if (findstr(get(mehc,'label'),'inactive'))
      ...
      set(ppdisp,'WindowButtonDownFcn','pplane8b(''down'')');
...
elseif strcmp(action,'down')
  ...
  pplane8b('solution',initpt,disph);
...
elseif strcmp(action,'test case')
   ...
   for k = 1:10
      initpt = k*[-.2 .2];
      pplane8b('solution',initpt,ppdisp);
      initpt = k*[-.2 -.2];
      pplane8b('solution',initpt,ppdisp);
   end  
   pplane8b('solution',[1,-1],ppdisp);
   dud = get(gcf,'UserData');
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
   nstr{5,1} = 'Ready.';
...
elseif strcmp(action,'kcompute')
   ...
   % button on the pplane8b Keyboard figure.
   ...
   for kk = 1:6;
	 ...
	 xstr = pplane8b('paraeval',pn,pv,xstr);
	 ystr = pplane8b('paraeval',pn,pv,ystr);
       end
     end
   end
   xvalue = str2num(xstr);
...
elseif strcmp(action,'kbd')
   ...
   % sets up the pplane8b Keyboard figure which allows accurate input of 
   ...
   ppkbd = findobj('name','pplane8b Keyboard input');
   if  ~isempty(ppkbd)
     delete(ppkbd);
   end
         
   ppkbd = figure('name','pplane8b Keyboard input',...
		  'vis','off',...
		  'NumberTitle','off','tag','pplane8b');
   
   pplane8b('figdefault',ppkbd);
   ...
   kbd.comp = uicontrol('style','push',...		
			'string','Compute','call','pplane8b(''kcompute'')');
...
%  z0 = ppginput(1);
  ...
  ppeqpt = findobj('name','pplane8b Equilibrium point data');
  
   if (isempty(ppeqpt))
     ppeqpt = figure('vis','off','resize','on',...
 		    'name','pplane8b Equilibrium point data',...
 		    'NumberTitle','off','tag','pplane8b');
     pplane8b('figdefault',ppeqpt);
...
elseif strcmp(action,'zoomin')
   ...
   set(gcf,'WindowButtonDownFcn','pplane8b(''zoom'')',...
...
elseif strcmp(action,'zoom')
   ...
   set(disph,'UserData',dud);
   set(disph,'WindowButtonDownFcn','pplane8b(''down'')',...
      'WindowButtonUpFcn','');
   pplane8b('dirfield',disph);
   ppset = findobj('name','pplane8b Setup');
   if isempty(ppset)
      pplane8b('confused');
   else
...
elseif strcmp(action,'showbar')
  ...
  if strcmp(state,'figure')
	...
	'clickedcallback','pplane8b(''print'');');
    switch name
     case 'pplane8b Display'
      set(findobj(sbfig,'tooltipstr','Zoom Out'),...
	  'clickedcallback',['pplane8b(''zoomback'');' fixtb]);
      set(findobj(sbfig,'tooltipstr','Zoom In'),...
	  'clickedcallback',['pplane8b(''zoomin'');' fixtb]);
      delete(findobj(sbfig,'tooltipstr','Rotate 3D'));
      
     case 'pplane8b Linearization'
...
elseif strcmp(action,'settings')
   ...
   % up the pplane8b Settings window, which allows the user to interactively
   ...
   setfig = findobj('name','pplane8b Settings');
   if ~isempty(setfig);
     delete(setfig);
   end
   
   setfig = figure('name','pplane8b Settings',...
		   'NumberTitle','off',...
		   'tag','pplane8b','vis','off');
   
   pplane8b('figdefault',setfig);
...
elseif strcmp(action,'setchange')
   ...
   % on the pplane8b Settings window.
...
elseif strcmp(action,'delete')
   ...
   trjh = dud.solhand;
   notice = dud.notice;
   if exist('notice')   % Display window
      nstr = get(notice,'string');
      nstr(1:4) = nstr(2:5);
...
elseif strcmp(action,'plotxyfig')  % Build the graph figure.
  ...
  ppxy = figure('NumberTitle','off',...
		'tag','pplane8b','visible','off');
  name = ['pplane8b t-plot # ',int2str(ppxy.Number)];
  ud.color = dud.color;
  ud.type = 0;
  set(ppxy,'name',name,'UserData',ud);
  pplane8b('figdefault',ppxy);
  ...
  set(findobj(fmenu,'label','Print Pre&view...'),'pos',6);
  set(findobj(fmenu,'label','&Print...'),'pos',7);
  qmenu = uimenu(fmenu,'label','Quit pplane8b',...
		 'call','pplane8b(''quit'')',...
		 'separator','on');
...
elseif strcmp(action,'plxy')
   ...
   switch type
    case 1 
      ...
      ylabel(xstring)
      set(r3dh,'vis','off')
      set(fig,'WindowButtonDownFcn','pplane8b(''plxybdf'')');
    case 2
...
elseif strcmp(action,'crop')
   ...
   hh = pplane8b('plotxyfig',type,ppxy);
   ud = get(hh(1),'UserData');
   ud.data = [t;x;y];
   set(hh(1),'UserData',ud);
   aud = get(hh(2),'UserData');
...
elseif strcmp(action,'plxybdf')
   ...
   dispa = gca;
   dispf = gcf;
   set(dispf,'windowbuttonmotionfcn','pplane8b(''plxybmf'')',...
      'windowbuttonupfcn','pplane8b(''plxybuf'')',...
      'inter','on');
...
elseif strcmp(action,'print')
   ...
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
   nstr{5} = 'Preparing to print the pplane8b Display Window.  Please be patient.';
   set(dud.notice,'string',nstr);
   
   nstr(1:4) = nstr(2:5);
   nstr{5} = 'Printing the pplane8b Display Window.';
   set(dud.notice,'string',nstr);
...
elseif strcmp(action,'figdefault')
  ...
  fig = input1;
  set(fig,'CloseRequestFcn','pplane8b(''closefcn'')');
  ppset = findobj('name','pplane8b Setup');
  sud = get(ppset,'UserData');
  ud = get(fig,'UserData');
  ud.ssize = sud.ssize;
...
elseif strcmp(action,'zoominsq')
   ...
   set(gcf,'WindowButtonDownFcn','pplane8b(''zoomsqd'')',...
      'WindowButtonUpFcn','1;','inter','on');
   set(gca,'inter','on');
   dud = get(gcf,'UserData');
   nstr = get(dud.notice,'string');
...
elseif strcmp(action,'zoomsqd')
   ...
   set(ppdispa,'UserData',aud);
   set(gcf,'windowbuttonmotionfcn','pplane8b(''zoomsqm'')',...
      'windowbuttonupfcn','pplane8b(''zoomsqu'')');
...
elseif strcmp(action,'zoomsqu')
   ...
   set(disph,'UserData',dud);
   set(disph,'WindowButtonDownFcn','pplane8b(''down'')',...
      'WindowButtonMotionFcn','pplane8b(''cdisp'')',...
      'WindowButtonUpFcn','');
   pplane8b('dirfield',disph);
   ppset = findobj('name','pplane8b Setup');
   if isempty(ppset)
...
elseif strcmp(action,'level')
  ...
  pplevel = figure('name','pplane8b Level sets',...
		 'vis','off',...
		 'NumberTitle','off','tag','pplane8b');
  
  pplane8b('figdefault',pplevel);
...
elseif strcmp(action,'levcomp')
  ...
  for kk = 1:4;
	...
	pv(find(abs(pv)==32))=[];
	lfcn = pplane8b('paraeval',pn,pv,lfcn);
      end
    end
  end
  l = length(lfcn);
...
elseif strcmp(action,'closefcn')
   ...
   fig = gcf;  
   name = get(fig,'name');
   if strcmp(name,'pplane8b Setup') | strcmp(name,'pplane8b Display')
      quest = ['Closing this window will cause all pplane8b ',...
            'windows to close, and pplane8b will stop.  ',...
            'Do you want to quit pplane8b?'];
      butt = questdlg(quest,'Quit pplane8b?','Quit','Cancel','Quit');
      if strcmp(butt,'Quit')
         pplane8b('quit');
      end
   elseif strcmp(name,'pplane8b Linearization')
...
elseif strcmp(action,'confused')
   ...
   tstring = 'pplane8b is totally confused';
   qstring = {['You will have to restart pplane8b from '...
      ...
      'What do you want to do?'};
   bstr1 = 'Quit and restart pplane8b.';
   bstr2 = 'Just quit pplane8b.';
   bstr3 = 'Do nothing.';
   answer = questdlg(qstring,tstring,bstr1,bstr2,bstr3,bstr1);
   if strcmp(answer,bstr1)
      delete(findobj('tag','pplane8b'));
      pplane8b;return
...
% Input the user data. 
  ...
  tout = tout(1:N);
  yout = yout(1:N,:);
  if exist('dud.notice') & exist('dud.noticeflag')
    nstr = get(dud.notice,'string');
...
% PPOUT  is the output function for pplane8b.

%  Copywright (c)  John C. Polking, Rice University
%  Last Modified: January 21, 2001

output = 0;
ppdisp = findobj(get(0,'child'),'flat','name','pplane8b Display'); %gca;   % 
dud = get(ppdisp,'UserData');
ppdispa = dud.axes;
dfcn = dud.function;
